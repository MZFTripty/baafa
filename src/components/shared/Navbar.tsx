            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-40 md:hidden bg-[#B22234] pt-16"
                    >
                        <div className="flex h-full flex-col p-8 bg-[#B22234]">
                            <div className="flex flex-col gap-4 mb-10 pt-8">
                                {topActions.map((action) => (
                                    <Button key={action.name} variant={action.variant} className={cn(
                                        "w-full h-14 text-sm font-black uppercase tracking-widest rounded-xl",
                                        action.name === "Join" ? "bg-white text-[#B22234]" : "bg-white/10 text-white border-white border"
                                    )} asChild>
                                        <Link href={action.href} onClick={() => setIsOpen(false)}>{action.name}</Link>
                                    </Button>
                                ))}
                            </div>
                            <div className="space-y-2 border-t border-white/20 pt-8 flex flex-col items-center">
                                {navigation.map((item) => (
                                    <Link
                                        key={item.name}
                                        href={item.href}
                                        className={cn(
                                            "w-full text-center py-4 text-xl font-black uppercase tracking-[0.3em]",
                                            pathname.startsWith(item.href)
                                                ? "text-baafa-gold"
                                                : "text-white hover:text-baafa-gold"
                                        )}
